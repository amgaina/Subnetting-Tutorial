...
let subnets = [128, 192, 224, 240, 248];

/* Stores the last number for the subnet mask*/
let subnet;

/* Stores the last number for the network id, broadcast id.*/
let first, last;

/* Stores the index at which our information corresponds in the hack table */
...
function rand254() {
  ...
  isValidNumber = isRandomValid(num);
  while (!isValidNumber) {
 ...
 *
 * @param {number} number
 * @returns if the chosen random number is valid
 */
function isRandomValid(number) {
  for (let i = 0; i < group_sizes.length; i++) {
    const num = group_sizes[i];
    const remainder = number % num;
    if (remainder === 0 || remainder === num - 1) {
      return false;
    }
  }
  return true;
 ...
 */
function generateIP() {
  ip[0] = Math.floor(Math.random() * (223 - 192 + 1)) + 192;
...
function showIP() {
  ...
  getclass.innerHTML =
    "Lets assume that your computer (i.e host device) has been assigned the following IP address and the CIDR value shown below : " +
...
function next0() {
  ...
  check(default_sub, "C", feedback_mssg0);
  id_1.style.display = default_sub === "C" ? "block" : "none";
}
...
function next1_b() {
  ...
  let value =
    parseInt(ip[0], 10).toString(2).padStart(8, "0") +
    "." +
    parseInt(ip[1], 10).toString(2).padStart(8, "0") +
    "." +
    parseInt(ip[2], 10).toString(2).padStart(8, "0") +
    "." +
    parseInt(ip[3], 10).toString(2).padStart(8, "0");
...
function next2() {
  ...
  let value = ip[0] + "." + ip[1] + "." + ip[2] + "." + first;
...
function next3() {
  ...
  let value = ip[0] + "." + ip[1] + "." + ip[2] + "." + last;

  if (last >= 256) {
    last = 255;
    value = ip[0] + "." + ip[1] + "." + ip[2] + "." + "255";
  }
...
function next4() {
  ...
  let value = ip[0] + "." + ip[1] + "." + ip[2] + "." + (first + 1);
...
function next5() {
  ...
  let value = ip[0] + "." + ip[1] + "." + ip[2] + "." + (last - 1);
...
function next6() {
  ...
  refreshButton.style.display =
    parseInt(usable_host) === value ? "block" : "none";
}
